...
void Function(Map<String, dynamic> pose)? onCameraPositionUpdate;
...
Future<Map<String, dynamic>?> getCameraPose() async {
  try {
    // Вызываем нативный метод, который вы нашли в Java
    final Map<dynamic, dynamic>? pose = await _channel.invokeMethod('getCameraPose');
    if (pose != null) {
      return Map<String, dynamic>.from(pose);
    }
    return null;
  } catch (e) {
    print("Ошибка получения позиции камеры: $e");
    return null;
  }
}
  ...
  Future<dynamic> _handleMethodCalls(MethodCall call) async {
    ...
    switch (call.method) {
      case 'onCameraPositionUpdate':
  if (onCameraPositionUpdate != null) {
    final Map<String, dynamic> pose = Map<String, dynamic>.from(call.arguments);
    onCameraPositionUpdate!(pose);
  }
  break;
      case 'onError':
        if (onError != null) {
